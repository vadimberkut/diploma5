...
namespace diploma5_csharp
{
    public static class GammaCorrection
    {
        ...
        public static Image<Bgr, Byte> Adaptive(Image<Bgr, Byte> image, bool showWindows = false)
        {
            ...
            double mu = StatisticsHelper.Average(pixelValues); // mean of the image intensity
            double sigma = StatisticsHelper.StandartDeviation(pixelValues); // standard deviation
            ...
            Func<List<string>> g = () =>
            {
               ...
               // D=diff((μ+2σ),(μ−2σ))
               double D = (mu + 2 * sigma) - (mu - 2 * sigma);

               //  τ is a parameter used for defining the contrast of an image
               double tao = 3; // τ=3 is a suitable choice for characterizing the contrasts of different images.
            ...
            if (isLowContrastImage)
            {
                ...
                // calc k
                double I_gamma = Math.Pow(MathHelper.VectorLength(pixelValues), gamma);
                double k = I_gamma + (1 - I_gamma) * Math.Pow(mu, gamma);

                // calc c
                c = 1.0 / (1 + Heaviside(0.5 - mu) * (k - 1));
            ...
            for (int m = 0; m < image.Rows; m++)
            {
                ...
                {
                    Gray pixel = grayImage[m, n];
                    double val = c * Math.Pow(pixel.Intensity, gamma);
                    grayImageEnhancedNormilized[m, n] = new Gray();
                }
            }
            ...
            if (showWindows)
            {
                EmguCvWindowManager.Display(image, "AGC_1 image");
                EmguCvWindowManager.Display(hsvImage, "AGC_2 hsvImage");
                EmguCvWindowManager.Display(grayImage, "AGC_3 grayImage");
                EmguCvWindowManager.Display(grayImage, "AGC_4 grayImageEnhanced");
                EmguCvWindowManager.Display(hsvImageEnhanced, "AGC_5 hsvImageEnhanced");
                EmguCvWindowManager.Display(result, "AGC_6 result");
            }

            return result;
        }

        public static Image<Bgr, Byte> AdaptiveV2(Image<Bgr, Byte> image, bool showWindows = false)
        {
            Image<Hsv, Byte> hsvImage;
            Image<Gray, Byte> grayImage;
            Image<Gray, double> grayImageNormilized;
            ...
            if (showWindows)
            {
                ...
                EmguCvWindowManager.Display(hsvImage, "AGC_2 hsvImage");
                EmguCvWindowManager.Display(grayImage, "AGC_3 grayImage");
                EmguCvWindowManager.Display(grayImageEnhanced, "AGC_4 grayImageEnhanced");
                EmguCvWindowManager.Display(hsvImageEnhanced, "AGC_5 hsvImageEnhanced");
                EmguCvWindowManager.Display(result, "AGC_6 result");
            }
